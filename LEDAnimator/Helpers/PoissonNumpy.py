...
ref: https://github.com/IHautaI/poisson-disc

code to implement Poisson Disc sampling

Used to create a starry night scene by generating a random list of coordinates (a chain)
which should not contain 'clumps'

usage:-

 g=Grid(radius,width,height)    # radius control the seperation, widtha nd height are the panel dimensions
 randseed= (random.uniform(0, length), random.uniform(0, width))
 list=g.poisson(randseed)

list is then [(x0,y0),....(xn,yn)]

TODO: find a numpy based alternative to speed it up. First call takes too long so not good for randomising.
